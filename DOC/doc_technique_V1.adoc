...
Cette méthode va appeller la fonction creerCompte provennant de la class CompteManagement, c'est cette dernière qui va s'occuper de l'ouverture de la fenêtre et qui va créer le compte du client en retournant ce compte à la class ComptesManagementController afin qu'elle puisse l'ajouter dans sa liste "ObservableList<CompteCourant>" permettant le bon affichage de tout les comptes du client.
...
Dans CompteManagement, c'est par l'appel de la fonction "doCompteEditorDialog()" que le compte est créer et si il n'est pas nul, le programme appelle la méthode "insertCompteCourant(compte)" provenant de la class AccessCompteCourant.

C'est dans cette méthode que le compte du client va être ajouté à la base donnée sur oracle par une requête sql reprennant les différents attributs du CompteCourant passé en paramètre.
