...
public class OperationEditorPaneController implements Initializable {
	...
	//attribut modifier lors de la validation d'un debit lors de l'activation d'un bouton
	//soit debit exceptionnel, soit normal
	...
	public Operation displayDialog(CompteCourant cpte, CategorieOperation mode) {
		...
		case DEBIT:
			...
			this.cbTypeOpe.setEditable(false);
	...
	/**
	 * Permet de valider un débit exceptionnel sur un compte. Assigner au bouton debit exceptionnel
	 */
	@FXML
	private void doDebitExceptionnel() {
		...
		if (this.compteEdite.solde - montant < this.compteEdite.debitAutorise) {
			//desactiver le fonctionnement du debitAutorise
		}
	...
	private void doAjouter() {
		if(this.txtMontant.getText().length()>6) {
			this.lblMessage.setText("Le champ montant ne doit pas contenir plus de 6 caractères");
			this.txtMontant.getStyleClass().add("borderred");
			this.lblMontant.getStyleClass().add("borderred");
			this.txtMontant.requestFocus();
			return;
	...
	/**Retourne si le debit actuel est un debit exceptionnel ou non
	 * 
	 * @return boolean
	 * */
	public boolean getIsDebitExceptionnel() {
		return this.IsDebitExceptionnel;
	}
}
