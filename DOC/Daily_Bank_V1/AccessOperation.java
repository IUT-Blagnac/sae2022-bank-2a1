...
public class AccessOperation {
	...
	public ArrayList<Operation> getOperations(int idNumCompte) throws DataAccessException, DatabaseConnexionException {
		...
		try {
			...
			while (rs.next()) {
				...
				alResult.add(new Operation(idOperation, montant, dateOp, dateValeur, idNumCompteTrouve, idTypeOp, null));
			}
			rs.close();
			pst.close();
			return alResult;
	...
	/**
	 * Enregistrement d'un débit/crédit (vérification dans la procédure stockée)
	 *
	...
	/**
	 * Enregistrement d'un débit exceptionnel (vérification du montant non nul dans la procédure stockée)
	 *
	...
	public void insertDebitExceptionnel(int idNumCompte, double montant, String typeOp)
		...
		try {
			...
			String q = "{call DEBITER_EXCEPTIONNEL (?, ?, ?, ?)}";
			// les ? correspondent aux paramètres : cf. déf procédure (4 paramètres)
			call = con.prepareCall(q);
			// Paramètres in
			call.setInt(1, idNumCompte);
	 ...
	 *
	 * Se fait par procédure stockée : Met à jour le solde des comptes.
	 *
	 * @param idNumCompteDeb compte débité
	 * @param idNumCompteCred compte crédité
	 * @param montant      montant débité
	 * @throws RowNotFoundOrTooManyRowsException
	 * @throws DataAccessException
	 * @throws DatabaseConnexionException
